...
class LabService:
    ...
    async def list_orders(
        self, page: int = 1, size: int = 20, status: str | None = None, patient_id: int | None = None, doctor_id: int | None = None
    ):
        skip = (page - 1) * size
        items = await self.order_repo.list_all(skip=skip, limit=size, status=status, patient_id=patient_id, doctor_id=doctor_id)
        total = await self.order_repo.count_all(status=status, patient_id=patient_id, doctor_id=doctor_id)
